/*
  Prosthetic Hand + BLE + Buttons + MAX17048 Fuel Gauge
...
void setup() {
  ...
  Serial.println("🔋 MAX17048 Fuel Gauge Test");

  if (!maxlipo.begin()) {
    Serial.println("⚠️ Could not find MAX17048!");
    // Continue anyway, but battery readings won't work
  } else {
    Serial.println("✅ MAX17048 detected!");
    maxlipo.quickStart();   // optional quick start
  }
...
void loop() {
  ...
  // ----- Battery monitoring and BLE notify every 5 sec -----
...
// ================== BATTERY FUNCTIONS ==================
float readBatteryVoltage() {
  float voltage = maxlipo.cellVoltage();  // Li-ion voltage (V)
  return voltage;
}

int readBatteryPercent() {
  float percent = maxlipo.cellPercent();  // battery percentage (%)
  return (int)percent;
}
...
void sendBattery() {
  ...
  int percent = readBatteryPercent();
  float voltage = readBatteryVoltage();
  String data = "BAT:" + String(percent) + "% V:" + String(voltage, 2);

  Serial.print("📤 Sending battery via BLE: ");
  Serial.println(data);
